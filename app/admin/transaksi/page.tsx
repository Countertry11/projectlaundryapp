...
import {
  ...
  transactionStatusSteps,
} from "@/lib/transactionProgress.mjs";
...
export default function TransaksiAdmin() {
  ...
  const syncDelayDiscounts = useCallback(async (
  ...
  }, [selectedOutletId, syncDelayDiscounts]);
  ...
  const fetchUserOutlet = useCallback(async () => {
    if (!user?.id) {
  ...
  useEffect(() => {
    ...
    void fetchUserOutlet();
  }, [fetchMasterData, fetchUserOutlet]);
  ...
  async function openTransactionDetail(trx: Transaction) {
    ...
    try {
      ...
      const detailRows = (
        ...
        const matchedPaket =
          packages.find((item) => item.harga === detail.price) || null;
  ...
  const currentStatusIndex = selectedTransaction
    ? transactionStatusSteps.findIndex(
        (step) => step.value === selectedTransaction.status,
      )
    : -1;
  const nextStatus = selectedTransaction
    ? getNextTransactionStatus(selectedTransaction.status)
    : null;
  ...
  const selectedTransactionFinancialSummary = selectedTransactionView
    ...
    : null;
  const pickupBlocked = selectedTransaction
    ? isTransactionPickupBlocked(selectedTransaction)
    : false;
    ...
    <AnimatedPage className="min-h-screen bg-[#f8fafc] p-6 space-y-6 font-sans text-slate-800">
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4 animate-fadeInUp">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-8 py-3.5 text-sm font-bold text-white shadow-md transition-all hover:bg-blue-700"
        >
          <Plus size={20} />
          Buat Transaksi Baru
        </button>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100">
                ...
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-gray-400 text-center">
                  Status
                </th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-gray-400 text-center">
                  Pembayaran
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-center">
                        <span
                          className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border inline-block text-center whitespace-nowrap ${transactionStatusColors[trx.status] || transactionStatusColors.pending}`}
                        >
                          {transactionStatusLabels[trx.status] || trx.status}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex justify-center">
                        <span
                          className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border inline-block text-center whitespace-nowrap ${
            ...
            >
              <div className="space-y-1.5 col-span-2">
                ...
                <p className="ml-1 text-[11px] font-medium text-slate-500">
                  Admin bisa memilih outlet tujuan transaksi langsung dari form ini.
                </p>
              </div>
            ...
            <div className="p-6 md:p-8 space-y-6 overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                ...
                <div className="rounded-3xl border border-slate-100 bg-slate-50 p-4">
                  ...
                  <span
                    className={`inline-flex px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${transactionStatusColors[selectedTransaction.status] || transactionStatusColors.pending}`}
                  >
                ...
                <div className="rounded-3xl border border-slate-100 bg-slate-50 p-4">
                  ...
                  <span
                    className={`inline-flex px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                      ...
                      >
                        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                            ...
                            <div className="mt-2 text-sm text-slate-500 space-y-1">
                              <div>
                                ...
                                <span className="font-semibold text-slate-700">
                                  {detail.paket?.jenis
                                    ? jenisLabels[detail.paket.jenis] || detail.paket.jenis
                                    : "-"}
                                </span>
                              </div>
                              <div>
                          ...
                          <div className="text-sm text-slate-600 md:text-right space-y-1">
                            <div>
                              Qty:{" "}
                              <span className="font-bold text-slate-800">{detail.quantity}</span>
                            </div>
                            <div>
                              Harga:{" "}
                              <span className="font-bold text-slate-800">{formatRupiah(detail.price)}</span>
                            </div>
                            <div>
                              Subtotal:{" "}
                              <span className="font-black text-blue-700">
              ...
              <div className="rounded-[28px] border border-slate-100 bg-gradient-to-r from-blue-50 to-indigo-50 p-5">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                  <div>
                    ...
                    <div className="text-sm font-bold text-amber-700">
                      {formatDateTime(selectedTransaction.due_date)}
                    </div>
                  </div>
                  <div>
                    <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">
              ...
              <div className="rounded-[28px] border border-slate-100 bg-white p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
                  {transactionStatusSteps.map((step, index) => {
                    ...
                    const actionEnabled =
                      ...
                      canAdvanceTransactionStatus(
                        ...
                        step.value as TransactionStatus,
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          updateStatus(selectedTransaction.id, step.value as TransactionStatus)
                        }
