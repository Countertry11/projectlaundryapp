  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 space-y-8 text-slate-800 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-100 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 tracking-tight">Dashboard Kasir</h1>
          <p className="text-gray-500 text-sm">Selamat datang, <span className="capitalize font-semibold text-emerald-600">{user?.full_name}</span></p>
        </div>
      </div>

      {/* Stats Cards - Ikon diperbaiki di sini */}
...
// Perbaikan StatCard: Menggunakan class solid sesuai screenshot
function StatCard({ title, value, icon: Icon, color, isCurrency }: any) {
  return (
    <div className="bg-white p-4 md:p-5 rounded-[24px] shadow-sm border border-gray-100 flex items-center gap-4 hover:shadow-md transition-all group min-w-0">
      {/* Wrapper Ikon: Dibuat solid sesuai screenshot */}
